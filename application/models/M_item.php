...
class M_item extends CI_Model {
    ...
    public function getBonanzaitem($Item_Id) {
        $query = $this->db->query("select * from auction_item where Item_Id='$Item_Id' ");
        return $query->row();
    }

    public function updateBonanzaitem($itemId, $itemType, $newImageName) {
        if (!empty($itemId) && !empty($itemType) && !empty($newImageName)):
            if ($itemType == 'ITEM-BACK'):
                $data = [];
                $data['Item_BackPicture'] = $newImageName;
                $this->db->where('Item_Id', $itemId);
                $query = $this->db->update('auction_item', $data);
                return $query;
            elseif ($itemType == 'ITEM-FRONT'):
                $data = [];
                $data['Item_FrontPicture'] = $newImageName;
                $this->db->where('Item_Id', $itemId);
                $query = $this->db->update('auction_item', $data);
                return $query;
            elseif ($itemType == 'ITEM-SIDE'):
                $data = [];
                $data['Item_SidePicture'] = $newImageName;
                $this->db->where('Item_Id', $itemId);
                $query = $this->db->update('auction_item', $data);
                return $query;
            elseif ($itemType == 'ITEM-INSIDE'):
                $data = [];
                $data['Item_InsideTagPicture'] = $newImageName;
                $this->db->where('Item_Id', $itemId);
                $query = $this->db->update('auction_item', $data);
                return $query;
            elseif ($itemType == 'ITEM-BOX'):
                $data = [];
                $data['Item_BoxPicture'] = $newImageName;
                $this->db->where('Item_Id', $itemId);
                $query = $this->db->update('auction_item', $data);
                return $query;
            endif;
