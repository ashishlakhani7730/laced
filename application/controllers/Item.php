...
class Item extends CI_Controller {
	...
	public function adddate()
	{
		if ($this->session->userdata('Admin_Id')) 
		{
            $this->load->view('auction_date');
        } else {
            redirect('login');
        }
	}

    public function viewAllProductImages() {
        $getItemId = $this->uri->segment(3);
        $getItem = $this->m_item->getBonanzaitem($getItemId);
        $this->load->view('instabuy_item_image_details', ['productData' => $getItem]);
    }

    public function viewAllAuctionProductImages() {
        $getItemId = $this->uri->segment(3);
        $getItem = $this->m_item->getBonanzaitem($getItemId);
        $this->load->view('aution_item_image_details', ['productData' => $getItem]);
    }

    public function viewAllTradeProductImages() {
        $getItemId = $this->uri->segment(3);
        $getItem = $this->m_item->getBonanzaitem($getItemId);
        $this->load->view('trade_item_image_details', ['productData' => $getItem]);
    }

    public function viewAllAllProductImages() {
        $getItemId = $this->uri->segment(3);
        $getItem = $this->m_item->getBonanzaitem($getItemId);
        $this->load->view('trade_item_image_details', ['productData' => $getItem]);
    }

    public function getTransparentImage() {

        $itemNo = $_REQUEST['itemId'];
        $itemType = $_REQUEST['type'];
        $getItem = $this->m_item->getBonanzaitem($itemNo);

        $transparentUrl = '';
        $type = '';
        if ($itemType == 'ITEM-BACK'):
            $transparentUrl = bonanzaImageUploadURL($getItem->Item_BackPicture);
            $type = 'ITEM-BACK';
        elseif ($itemType == 'ITEM-FRONT'):
            $transparentUrl = bonanzaImageUploadURL($getItem->Item_FrontPicture);
            $type = 'ITEM-FRONT';
        elseif ($itemType == 'ITEM-SIDE'):
            $transparentUrl = bonanzaImageUploadURL($getItem->Item_SidePicture);
            $type = 'ITEM-SIDE';
        elseif ($itemType == 'ITEM-INSIDE'):
            $transparentUrl = bonanzaImageUploadURL($getItem->Item_InsideTagPicture);
            $type = 'ITEM-INSIDE';
        elseif ($itemType == 'ITEM-BOX'):
            $transparentUrl = bonanzaImageUploadURL($getItem->Item_BoxPicture);
            $type = 'ITEM-BOX';
        endif;
//        $transparentUrl = 'https://images-na.ssl-images-amazon.com/images/I/71tYXYR6E-L._UL1500_.jpg';

        $newImageName = makeImageTransparent($transparentUrl);
        if (empty($newImageName)):
            print_r('Sorry, image not transparent. Please try again...!');
        else:
            $isUpdate = $this->m_item->updateBonanzaitem($itemNo, $itemType, $newImageName);
            if (!empty($isUpdate)):
                print_r('1');
            else:
                print_r('Your image updating error.');
               // $getItem = $this->m_item->getBonanzaitem($itemNo);
            endif;
        endif;
    }
