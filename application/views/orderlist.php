                ...
                <div class="row">
                    <div class="col-12 text-white p-t-40 p-b-90">
                        <h4 class=""> <span class="btn btn-white-translucent">
                                <i class="mdi mdi-table "></i></span> Order List
                        </h4>
                    </div>
                </div>
            </div>
										...
										foreach ($order_list as $item) {
											...
											<tr>
												...
												<td><div class="input-group-btn">
													<button type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown">Change Order Status <span class="caret"></span></button>
													<ul class="dropdown-menu">   
														
															<li><a onclick="deliverd('<?php echo $item->Order_Id; ?>')" href="JavaScript:void(0);"><span class="glyphicon glyphicon-remove" style="color: red"> </span>&nbsp; Delivered To Laced | Verified Authentic</a></li>
															
														
															<li><a onclick="shippingyou('<?php echo $item->Order_Id; ?>')" href="JavaScript:void(0);"><span class="glyphicon glyphicon-ok" style="color: green"> </span>&nbsp; Shipping You</a></li>
															
														<li> </li>
														<li></li>
														<li> <a  onclick="complete('<?php echo $item->Order_Id; ?>')" href="JavaScript:void(0);" ><span class="fa fa-trash-o"> &nbsp;    Delivered | Complete</span></a> </li>
...
<!--                                                                <td><div class="input-group-btn">
														<button type="button" class="btn btn-info dropdown-toggle" data-toggle="dropdown">Tracking <span class="caret"></span></button>
														<ul class="dropdown-menu">                                 

															<li><button type="button" class="form-control"  onclick="test('<?= @$item->Order_Id ?>')">Seller Tracking</button></li>

															<li><button type="button" class="form-control"  onclick="test1('<?= @$item->Order_Id ?>')">Buyer Tracking</button></li>
...
<script>
    ...
    {
        $.ajax({
            url: '<?php echo site_url('order_list/moredetails') ?> ',
            type: 'post',
            dataType: 'json',
...
<?php } ?>
	...
	{
		$.ajax({
			url: "<?php echo site_url("Order_list/deliverd") ?>",
			type: 'POST',
			data: {Order_Id: oid},
			//dataType: 'json'
			success: function (json) {
	...
	function shippingyou(oid)
	{
		$.ajax({
			url: "<?php echo site_url("Order_list/shippingyou") ?>",
			type: 'POST',
			data: {Order_Id: oid},
			//dataType: 'json'
			success: function (json) {
	...
	{
		$.ajax({
			url: "<?php echo site_url("Order_list/complete") ?>",
			type: 'POST',
			data: {Order_Id: oid},
			//dataType: 'json'
			success: function (json) {
