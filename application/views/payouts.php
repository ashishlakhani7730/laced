...
<section class="admin-content">
                ...
                <div class="row">
                    <div class="col-12 text-white p-t-40 p-b-90">
                        <h4 class=""> <span class="btn btn-white-translucent">
                                <i class="mdi mdi-table "></i></span> Auction Seller Payouts
                        </h4>
                    </div>
                </div>
            </div>
									...
									if($item_seller)
									{
										...
										foreach ($item_seller as $item) {
											...
											<tr>
												...
												<td><button id="more<?php echo $cnt++ ?>" type="button" class="btn btn-primary" onclick="moredetail(<?php echo $item->Item_Id; ?>)" data-toggle="modal" data-target="#moreresponsive-modal" data-whatever="@mdo">More</button></td>
												<td><button id="bid<?php echo $cnt++ ?>" type="button" class="btn  btn-success" onclick="getbiding(<?php echo $item->Item_Id; ?>)" data-toggle="modal" data-target="#bidresponsive-modal" data-whatever="@mdo"><i class="fa fa-gavel mr-20"></i>Biding</button></td>
												<td><button id="payouts<?php echo $cnt++ ?>" type="button" class="btn  btn-success" onclick="paidpayout(<?php echo $item->User_Id; ?>,<?php echo $item->Bid_id; ?>,<?php echo $item->Bid_Price; ?>)"><i class="fa fa-gavel mr-20"></i>Pay</button></td>
											</tr>
											<?php
										}
									}
		...
		<div class="modal fade" id="pleasewait" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
...
<script>
	...
	function paidpayout(user_id,bid_id,bid_price)
	{
		$("#pleasewait").modal("show");
		$.ajax({
				url: '<?php echo site_url('bid/pay_stripe') ?> ',
				type: 'post',
				dataType: 'json',
				data: {user_id:user_id,bid_id:bid_id,bid_price:bid_price},
				success: function(json) {
